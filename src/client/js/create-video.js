...
document.addEventListener("DOMContentLoaded", () => {
    const selectFileButton = document.querySelector("#select-files-button");
    const fileDataForm = document.querySelector("#filedata-form");
    const fileDataInput = fileDataForm.querySelector("#filedata-input");
    
    selectFileButton.addEventListener("click", () => fileDataInput.click());
    fileDataInput.addEventListener("change", () => {
        if (fileDataInput.files.length > 0) {
            fileDataForm.submit();
        };
    });
    fileDataForm.addEventListener("submit", () => {
        ...
        const submitButton = document.querySelector("button[aria-label='저장']");
